...
def get_dataloader(num_train=0, valid=0.1, num_test=0, label="fine"):
    ...
    if label == "fine":
        ...
        (x_train, y_train_coarse), (x_test, y_test_coarse) = cifar100.load_data(
            "coarse"
        )
        y_train_fine, y_test_fine = one_hot_encoding(y_train_fine, y_test_fine, 100)
        y_train_coarse, y_test_coarse = one_hot_encoding(
            y_train_coarse, y_test_coarse, 20
        )
        y_train = np.concatenate(
            [y_train_fine[..., None], y_train_coarse[..., None]], 1
        )
    ...
    return {
        ...
        "valid": (x_val, y_val),
        "test": (x_test[:num_test], y_test[:num_test]),
    }
