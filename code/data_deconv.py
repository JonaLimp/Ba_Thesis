...
def get_top_k_activation(model, data, k):
    ...
    layer_outputs = [
        layer.output for layer in model.layers[1:] if isinstance(layer, layers.Conv2D)
    ]
    ...
    all_activations = np.zeros(
        (len(data), len(layer_outputs), int(layer_outputs[-1].shape[-1]))
    )
        ...
        for index, elem in enumerate(img_dict[sample]):
            ...
            if isinstance(deconv_layers[layer_idx[elem[1]]][0].layer, layers.Conv2D):
                ...
                if elem[1] in deconv_dict.keys():
                    ...
                    num_feature_maps = (
                        deconv_layers[layer_idx[elem[1]]][0]
                        .layer.get_weights()[0]
                        .shape[3]
                    )
...
def translate_representations(
    deconv_save_path, layer_list, model, num_neurons, steps, label, data
):
    ...
    for key, n_list in deconv.items():
        ...
        for neuron_idx in rand_neurons:
            ...
            if not np.any(act_array):
                act_array, other_neurons, neuron_characteristics = shift_non_zeros(
                    n_list, model, key, steps, other_neurons
                )
            if not np.any(act_array):
                ipdb.set_trace()
            ...
            print(
                "FWHM of representation: {} and actual mean: {}".format(
                    FWHM, np.mean(FWHM_list)
                )
            )
            print(
                "Slope of representation: {} and actual mean: {}".format(
                    linreg, np.mean(linreg_list)
                )
            )
        ...
        print(
            "layer {} translated with {} randomly chosen neurons and {} steps.".format(
                key, num_neurons, steps
            )
        )

    res = [layer_FWHM_dict, layer_linreg_dict, neuron_characteristics_dict]

    trans_rep_save_path = os.path.join(
        dir_path, "trans_rep_all_steps_{}.pickle".format(label)
    )
    ipdb.set_trace()
...
def shift_and_activate(rep, model, layer, neuron_idx, steps):
    ...
    heights, widths = (
        model.get_layer(layer).output.shape[1],
        model.get_layer(layer).output.shape[2],
    )
    ...
    for direction in directions:
        for step in range(1, steps + 1):
            ...
            act_array[cord[0] + steps, cord[1] + steps] = out[
                ..., neuron_idx
            ].flatten()[max_idx]
...
def calculate_kolmogrov(dir_path, deconv_save_path, label):
    ...
    kolmogrov_save_path = os.path.join(
        dir_path, "kolmogrov_mean_{}.pickle".format(label)
    )
...
if __name__ == "__main__":
    ...
    activation_save_path = os.path.join(
        dir_path, "activation_dict_{}.pickle".format(label)
    )
