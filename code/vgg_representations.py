...
def VGG16_representation(img_name):
    ...
    activation_save_path = os.path.join(
        dir_path, "activation_dict_VGG_{}.pickle".format(img_name)
    )
    deconv_save_path = os.path.join(
        dir_path, "deconv_dict_VGG_{}.pickle".format(img_name)
    )
    trans_rep_save_path = os.path.join(
        dir_path, "trans_rep_dict_VGG_{}.pickle".format(img_name)
    )
...
def visualize_vgg_representations(img_name):
    ...
    pdb.set_trace()
    img_df = pd.read_csv(
        csv_save_path, names=["id", "sample", "layer", "neuron", "activation"]
    )
